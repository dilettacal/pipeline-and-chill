...
def print_summary_stats(df: pd.DataFrame, month_str: str):
    ...
    print(
        f"   {'Quantile':<10} {'Distance (km)':<15} {'Duration (min)':<15} {'Speed (km/h)':<15} {'Fare ($)':<10}"
    )
    print(f"   {'-'*70}")
    for q in quantiles:
...
def plot_distributions(df: pd.DataFrame, month_str: str, save: bool = False):
    ...
    axes[0, 2].axvline(
        df["avg_speed_kmh"].median(),
        color="red",
        linestyle="--",
        label=f'Median: {df["avg_speed_kmh"].median():.1f}km/h',
    )
    axes[0, 2].legend()
    ...
    axes[1, 0].axvline(
        ...
        color="red",
        linestyle="--",
        label=f'Median: ${df["fare_amount"].median():.1f}',
    )
    axes[1, 0].legend()
